...
table.post("/rename-table", requireUser, async (c) => {
  try {
    let { connection, oldTableName, newTableName, provider } =
      await c.req.json();
    const userId = c.get("userId");

    if (
      provider === "surrealdb" ||
      (connection && connection.provider === "surrealdb")
    ) {
      provider = "postgres";
    }

    const requestId = c.get("requestId");
    logger.info(`[Rename] Received rename request`, {
      oldTableName,
      newTableName,
      provider,
      requestId,
    ...
    if (provider === "sqlite" && tursoPath?.includes("turso.io")) {
      const uuidMatch = oldTableName.match(
        /data_([a-f0-9]{8}_[a-f0-9]{4}_[a-f0-9]{4}_[a-f0-9]{4}_[a-f0-9]{12})_/,
      );
      const uuidWithUnderscores = uuidMatch ? uuidMatch[1] : null;
      const uploadId = uuidWithUnderscores
        ? uuidWithUnderscores.replace(/_/g, "-")
        : null;
      ...
      const uuidMatch = oldTableName.match(
        /^data_([a-f0-9]{32}|[a-f0-9]{8}_[a-f0-9]{4}_[a-f0-9]{4}_[a-f0-9]{4}_[a-f0-9]{12})_/i,
      );
      if (uuidMatch) {
    ...
    try {
      ...
      if (connection.id) {
        ...
        let connRow = null;
        if (
          uuidRegex.test(connection.id) &&
          !connection.id.startsWith("system:")
        ) {
...
table.post("/delete-table", async (c) => {
  try {
    ...
    if (provider === "sqlite" && tursoPath?.includes("turso.io")) {
      const uuidMatch = tableName.match(
        /data_([a-f0-9]{8}_[a-f0-9]{4}_[a-f0-9]{4}_[a-f0-9]{4}_[a-f0-9]{12})_/,
      );
    ...
    try {
      ...
      if (connection.id) {
        const uuidRegex =
          /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
        const connRow =
          uuidRegex.test(connection.id) && !connection.id.startsWith("system:")
  ...
  try {
    ...
    if (provider === "sqlite" && connection?.url?.includes("turso")) {
      ...
      if (uploadIdMatch) {
        ...
        try {
          ...
          logger.error("[Save] Error verifying upload ownership", err, {
    ...
    try {
      ...
      if (provider === "duckdb" || provider === "file") {
        const requestId = c.get("requestId");
        logger.debug(`[Table Save] Mapping table. Requested: ${tableName}`, {
        ...
        if (
          connection &&
          connection.tables &&
          Array.isArray(connection.tables)
        ) {
          ...
          const matchedTable = connection.tables.find((t) => {
            ...
            return (
              tSanitized === targetSanitized ||
              tSanitized.includes(targetSanitized) ||
              targetSanitized.includes(tSanitized)
            );
          });
          ...
          if (matchedTable) {
            ...
            logger.debug(
              `[Table Save] Mapped table name via fuzzy match: ${tableName} -> ${actualTableName}`,
              { requestId },
            );
          }
          // Strategy 2: Single Table Fallback
            ...
            logger.debug(
              `[Table Save] Fallback to single available table: ${actualTableName}`,
              { requestId },
            );
          }
        }
      ...
      for (const rowid of deletedRowIds) {
        if (rowid)
          queries.push(
            `DELETE FROM "${actualTableName}" WHERE rowid = ${Number(rowid)}`,
          );
      }
      for (const col of deletedColumns) {
        if (col && col !== "_rowid_")
          queries.push(`ALTER TABLE "${actualTableName}" DROP COLUMN "${col}"`);
      }
      ...
      for (const update of updates) {
        ...
        const idKey = Object.keys(rowData).find(
          (k) => k === "_rowid_" || k.toLowerCase().endsWith("id"),
        );
        ...
        if (idKey && rowData[idKey] !== undefined) {
          const setClause = Object.keys(rowData)
            ...
            .map(
              (k) =>
                `"${k}" = ${rowData[k] === null ? "NULL" : `'${String(rowData[k]).replace(/'/g, "''")}'`}`,
            )
            .join(", ");
          if (setClause)
            queries.push(
              `UPDATE "${actualTableName}" SET ${setClause} WHERE "${idKey}" = '${String(rowData[idKey]).replace(/'/g, "''")}'`,
            );
          ...
          const whereClause = Object.keys(originalData)
            ...
            .map(
              (k) =>
                `"${k}" ${originalData[k] === null ? "IS NULL" : `= '${String(originalData[k]).replace(/'/g, "''")}'`}`,
            )
            .join(" AND ");
          if (setClause && whereClause)
            queries.push(
              `UPDATE "${actualTableName}" SET ${setClause} WHERE ${whereClause}`,
            );
        } else {
          ...
          if (keys.length) {
            ...
            const vals = keys
              .map((k) =>
                rowData[k] === null
                  ? "NULL"
                  : `'${String(rowData[k]).replace(/'/g, "''")}'`,
              )
              .join(", ");
            queries.push(
              `INSERT INTO "${actualTableName}" (${cols}) VALUES (${vals})`,
            );
          }
        }
      }
...
table.post("/copy-table", async (c) => {
  try {
    let { sourceTableName, newTableName, connection, provider } =
      await c.req.json();
    if (
      provider === "surrealdb" ||
      (connection && connection.provider === "surrealdb")
...
table.post("/table/:tableName/schema", async (c) => {
  try {
    ...
    const requestId = c.get("requestId");
    logger.info("[Table Schema] Request", {
      tableName,
      provider,
    ...
    if (!provider) {
      provider = "duckdb";
      logger.debug(
        "[Table Schema] Provider was undefined, defaulting to duckdb",
        { requestId: c.get("requestId") },
      );
    }
    ...
    try {
      ...
      if (provider === "duckdb" || provider === "file") {
        console.log(
          `[Table Schema] Mapping table. Requested: ${tableName}. Connection tables:`,
          connection?.tables ? JSON.stringify(connection.tables) : "undefined",
        );
        ...
        if (
          ...
          if (matchedTable) {
            actualTableName = matchedTable;
            console.log(
              `[Table Schema] Mapped table name via fuzzy match: ${tableName} -> ${actualTableName}`,
            );
          }
          // Strategy 2: Single Table Fallback
          else if (connection.tables.length === 1) {
            actualTableName = connection.tables[0];
            console.log(
              `[Table Schema] Fallback to single available table: ${actualTableName}`,
            );
          }
        }
      }
...
table.post("/table/:tableName/query", async (c) => {
  try {
    ...
    console.log("[Table Query] Request:", {
      tableName,
      provider,
      hasConnection: !!connection,
      limit,
    ...
    if (
      provider === "cosmosdb" &&
      (tableName === "OrionMetrics" || tableName === "orionmetrics") &&
      process.env.COSMOS_ENDPOINT
    ) {
      if (!connection) connection = {};
      if (!connection.endpoint)
        connection.endpoint = process.env.COSMOS_ENDPOINT;
    ...
    try {
      ...
      if (provider === "duckdb" || provider === "file") {
        console.log(
          `[Table Query] Mapping table. Requested: ${tableName}. Connection tables:`,
          connection?.tables ? JSON.stringify(connection.tables) : "undefined",
        );
        ...
        if (
          ...
          if (matchedTable) {
            actualTableName = matchedTable;
            console.log(
              `[Table Query] Mapped table name via fuzzy match: ${tableName} -> ${actualTableName}`,
            );
          }
          // Strategy 2: Single Table Fallback
          else if (connection.tables.length === 1) {
            actualTableName = connection.tables[0];
            console.log(
              `[Table Query] Fallback to single available table: ${actualTableName}`,
            );
          }
        }
      }
        ...
        if (op.type === "full_replacement") {
          ...
          if (provider === "surrealdb") {
            ...
            for (let i = 0; i < rows.length; i += batchSize) {
              ...
              if (batch.length > 0) {
                queries.push(
                  `INSERT INTO ${tableName} ${JSON.stringify(batch)}`,
                );
              }
            }
          } else {
          ...
          const setClause = Object.keys(changes)
            .map(
              (k) =>
                `"${k}" = ${changes[k] === null ? "NULL" : `'${String(changes[k]).replace(/'/g, "''")}'`}`,
            )
            .join(", ");
          ...
          queries.push(
            `UPDATE "${tableName}" SET ${setClause} WHERE "${idCol}" = '${String(id).replace(/'/g, "''")}'`,
          );
          ...
          const idCol = op.idColumn || "id";
          queries.push(
            `DELETE FROM "${tableName}" WHERE "${idCol}" = '${String(op.id).replace(/'/g, "''")}'`,
          );
        } else if (op.type === "add_column") {
          queries.push(
            `ALTER TABLE "${tableName}" ADD COLUMN "${op.column}" TEXT`,
          );
...
table.post("/table/:tableName/sanitize", async (c) => {
  try {
    ...
    return c.json({
      success: true,
      message: "Sanitization stub - logic being migrated",
    });
  } catch (e) {
    return c.json({ error: e.message }, 500);
  }
...
table.get("/:tableName/interpret", async (c) => {
  try {
    ...
    try {
      ...
      if (provider === "duckdb" || provider === "file") {
        console.log(
          `[Table Interpret] Mapping table. Requested: ${tableName}. Connection tables:`,
          connection?.tables ? JSON.stringify(connection.tables) : "undefined",
        );
        ...
        if (
          ...
          if (matchedTable) {
            actualTableName = matchedTable;
            console.log(
              `[Table Interpret] Mapped table name via fuzzy match: ${tableName} -> ${actualTableName}`,
            );
          }
          // Strategy 2: Single Table Fallback
          else if (connection.tables.length === 1) {
            actualTableName = connection.tables[0];
            console.log(
              `[Table Interpret] Fallback to single available table: ${actualTableName}`,
            );
          }
        }
      }
      ...
      const rows = await adapter.query(
        `SELECT * FROM "${actualTableName}" LIMIT 100`,
      );
      await adapter.disconnect();
    ...
    await db
      ...
      .onConflictDoUpdate({
        target: [
          ...
          spreadsheetPermissions.userEmail,
        ],
...
table.get("/:tableName/permissions", async (c) => {
  try {
    ...
    const perms = await db
      ...
      .where(eq(spreadsheetPermissions.spreadsheet, tableName));
...
table.post("/table/:tableName/load", async (c) => {
  try {
    ...
    try {
      ...
      if (provider === "duckdb" || provider === "file") {
        console.log(
          `[Table Load] Mapping table. Requested: ${tableName}. Connection tables:`,
          connection?.tables ? JSON.stringify(connection.tables) : "undefined",
        );
        ...
        if (
          ...
          if (matchedTable) {
            actualTableName = matchedTable;
            console.log(
              `[Table Load] Mapped table name via fuzzy match: ${tableName} -> ${actualTableName}`,
            );
          }
          // Strategy 2: Single Table Fallback
          else if (connection.tables.length === 1) {
            actualTableName = connection.tables[0];
            console.log(
              `[Table Load] Fallback to single available table: ${actualTableName}`,
            );
          }
        }
      }
          ...
          if (provider === "surrealdb") {
            ...
            try {
              ...
              if (e.message.includes("no such column: rowid")) {
                console.log(
                  `[Table Load] Table ${actualTableName} has no rowid, falling back to simple SELECT`,
                );
      ...
      await adapter.disconnect();
      return c.json({
        columns: fullSchema[actualTableName] || [],
