...
<template>
  <div class="flex items-center justify-between gap-4 pt-2 select-none">
    ...
    <div class="flex items-center justify-end gap-1 *:select-none">
      <span
        class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold border transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
        :class="isAccessible.aa ? 'bg-primary text-primary-foreground border-transparent hover:bg-primary/80' : 'bg-secondary text-secondary-foreground border-transparent hover:bg-secondary/80'"
      >
        <Check
          v-if="isAccessible.aa"
          class="size-3"
        />
        <X
          v-else
      ...
      <span
        class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold border transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
        :class="isAccessible.aaa ? 'bg-primary text-primary-foreground border-transparent hover:bg-primary/80' : 'bg-secondary text-secondary-foreground border-transparent hover:bg-secondary/80'"
      >
        <Check
          v-if="isAccessible.aaa"
          class="size-3"
        />
        <X
          v-else
