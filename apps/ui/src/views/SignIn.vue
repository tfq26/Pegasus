      ...
      <div class="w-full max-w-[32rem]">
        ...
        <div class="rounded-3xl border border-slate-300/60 bg-white/60 p-8 shadow-[0_20px_50px_rgba(15,23,42,0.15)] backdrop-blur-xl dark:border-white/10 dark:bg-black/45 dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          ...
          <div class="mb-8 text-center">
            ...
            <h1 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-zinc-50">Welcome to Pegasus</h1>
            <p class="mt-3 text-sm text-slate-600 dark:text-zinc-400">
              Sign in securely using WorkOS authentication
            </p>
          </div>

          <!-- Singular Premium Login Button -->
          <div class="space-y-4">
            <button
              type="button"
              ...
              @click="handleWorkOSLogin"
            >
              <Loader2 v-if="isLoggingIn" class="h-5 w-5 animate-spin" />
            ...
            <div class="text-center text-sm mt-6">
              <span class="text-slate-600 dark:text-zinc-400">New to Pegasus?</span>
              <button
                type="button"
                @click="goToSignup"
...
function goToSignup() {
  ...
  if (typeof redirect === 'string' && redirect) {
    router.push({ path: '/signup', query: { redirect } })
    return
  }
  router.push('/signup')
}
