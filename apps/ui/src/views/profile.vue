<template>
  <div class="min-h-screen bg-background text-foreground p-4 md:p-8 xl:p-12 transition-all duration-500">
    ...
    <div v-else class="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <!-- Header Section -->
      <div class="flex flex-col lg:flex-row items-center justify-between gap-8 pb-10 border-b border-border/50">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-8 w-full lg:w-auto">
           ...
           <div class="flex items-center gap-3">
             <button 
               ...
               @click="handleUpgrade"
               class="px-5 py-2.5 rounded-xl bg-primary text-primary-foreground text-xs font-bold hover:bg-primary/90 transition-all hover:scale-[1.02] shadow-lg shadow-primary/20"
             >
               Upgrade
             </button>
             <button 
               ...
               class="px-5 py-2.5 rounded-xl bg-secondary text-secondary-foreground text-xs font-bold hover:bg-secondary/80 border border-border transition-all hover:scale-[1.02]"
             >
               Manage Plan
             </button>
             <button
               @click="logout"
               class="px-5 py-2.5 rounded-xl border border-red-500/20 text-red-500 text-xs font-bold hover:bg-red-500/5 transition-all"
             >
               Sign Out
             </button>
           </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 items-start">
        ...
        <div class="space-y-6 order-2 lg:order-1">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
            <!-- Tokens Circle -->
            <Card class="flex flex-col items-center hover:border-primary/30 transition-colors p-6">
            ...
            <!-- Storage Circle -->
            <Card class="flex flex-col items-center hover:border-blue-500/30 transition-colors p-6">
                <div class="relative w-32 h-32 flex items-center justify-center mb-4">
                   <!-- Blurred background glow -->
        ...
        <div class="space-y-6 order-1 lg:order-2">
          ...
          <!-- Purchase Resources Card -->
          <Card class="relative overflow-hidden">
            <Tabs :default-value="subscriptionTier === 'free' ? 'storage' : 'tokens'" class="w-full">
              <CardHeader class="pb-2">
                <div class="flex items-center justify-between">
                  <div>
                    <CardTitle class="text-lg font-semibold">Purchase Resources</CardTitle>
                    <CardDescription>Scale your vault's capabilities on demand.</CardDescription>
                  </div>
                  <TabsList :class="[
                    'grid w-[200px]',
                    subscriptionTier === 'free' ? 'grid-cols-1 w-[100px]' : 'grid-cols-2'
                  ]">
                    <TabsTrigger v-if="subscriptionTier !== 'free'" value="tokens">Tokens</TabsTrigger>
                    <TabsTrigger value="storage">Storage</TabsTrigger>
                  </TabsList>
                </div>
              </CardHeader>

              <TabsContent value="tokens" v-if="subscriptionTier !== 'free'">
                <CardContent>
                  <div class="relative">
                    <div class="absolute top-0 right-0 p-3 opacity-10 pointer-events-none">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-24 w-24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v8"/><path d="M8 12h8"/></svg>
                    </div>
                    ...
                    <div class="bg-secondary/30 p-5 rounded-xl border border-secondary backdrop-blur-sm">
                      <div class="flex justify-between items-end mb-6">
                        ...
                        <div class="text-right">
                            ...
                            <span class="text-2xl font-bold text-primary flex items-center justify-end">
                              $<NumberTicker :value="calculatedTokenPriceNum" :decimal-places="2" />
                            </span>
                        </div>
                      </div>
                      ...
                      <div class="mb-6">
                        <NumberField 
                            ...
                            id="token-quantity"
                        />
                        <div class="flex justify-between text-[10px] text-muted-foreground mt-2 font-medium uppercase tracking-wider">
                            <span>Min: 100k</span>
                            <span>Max: 700k</span>
                        </div>
                      </div>
              ...
              <TabsContent value="storage">
                <CardContent>
                  <div class="relative">
                    <div class="absolute top-0 right-0 p-3 opacity-10 text-blue-500 pointer-events-none">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-24 w-24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v3"/><path d="M21 16v3a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-3"/><rect width="20" height="8" x="2" y="8" rx="2"/></svg>
                    </div>
                    ...
                    <div class="bg-blue-500/5 p-5 rounded-xl border border-blue-500/10 backdrop-blur-sm">
                      <div class="flex justify-between items-end mb-6">
                         ...
                         <div class="text-right">
                            <span class="block text-xs font-medium uppercase tracking-wider text-muted-foreground mb-1">Total Price</span>
                            <span class="text-2xl font-bold text-blue-600 flex items-center justify-end">
                              $<NumberTicker :value="calculatedStoragePriceNum" :decimal-places="2" />
                            </span>
                         </div>
                      </div>
                      ...
                      <div class="mb-6">
                        <div class="relative group">
                          <NumberField 
                            ...
                            :max="storageMaxGB"
                            id="storage-quantity"
                            label="Add Capacity (GB)"
                          />
                        </div>
                        <div class="flex justify-between items-center mt-3 px-1">
                            <div class="flex items-center gap-1.5">
                              <span class="h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse"></span>
                              <span class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{{ storagePricePerGB }} per GB Expansion</span>
                            </div>
                            <span class="text-[10px] text-muted-foreground font-medium">{{ storageMaxGB }}GB Max</span>
                        </div>
                      </div>
          ...
          <Card v-if="subscriptionTier === 'free'" class="bg-gradient-to-br from-indigo-50 to-violet-50 dark:from-slate-900 dark:to-slate-800 border-indigo-100 dark:border-slate-700 text-center shadow-sm">
