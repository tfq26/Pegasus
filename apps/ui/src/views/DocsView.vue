...
const fetchIndex = async () => {
  ...
  try {
    const data = await api.get<any>('/docs')
    ...
    if (!route.params.type && !route.params.slug) {
      const latestRelease = changelogs.value[0]
      const firstGuide = guides.value[0]
      if (latestRelease) {
        selectItem('release', latestRelease)
      } else if (firstGuide) {
        selectItem('guide', firstGuide)
      }
    } else if (route.params.type && route.params.slug) {
      const typeString = Array.isArray(route.params.type) ? route.params.type[0] : route.params.type
      const slugString = Array.isArray(route.params.slug) ? route.params.slug[0] : route.params.slug
      if (typeString && slugString) {
        activeType.value = typeString as any
        activeSlug.value = slugString
...
const fetchContent = async (type: 'guide' | 'release', slug: string) => {
  ...
  try {
    const endpoint = type === 'guide' ? `/docs/guides/${slug}` : `/docs/releases/${slug}`
...
watch(() => route.params, (params) => {
  if (params.type && params.slug) {
    activeType.value = params.type as any
    activeSlug.value = params.slug as string
    fetchContent(activeType.value, activeSlug.value)
  }
}, { immediate: true })
  ...
  <div class="flex h-full bg-background overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto">
      <div class="max-w-4xl mx-auto px-8 py-12">
        <div v-if="isLoadingContent" class="flex flex-col items-center justify-center py-40 gap-4">
          <div class="relative">
            <Loader2 class="w-12 h-12 animate-spin text-violet-500/20" />
            <Loader2 class="w-12 h-12 animate-spin text-violet-500 absolute top-0 left-0 [animation-delay:-0.5s]" />
          </div>
          <p class="text-sm text-muted-foreground animate-pulse font-medium">Coming right up...</p>
        </div>
        ...
        <div v-else class="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <!-- Markdown Content -->
          <div class="markdown-body prose dark:prose-invert max-w-none" v-html="content"></div>
